...
const JSON_FIELDS = ['abilities', 'save_proficiencies', 'skill_proficiencies', 'inventory', 'spells'];
...
charactersRouter.post('/', (req, res) => {
  ...
  if (typeof name !== 'string' || !name.trim()) {
    return res.status(400).json({ error: 'El personaje necesita un nombre' });
  }
  const info = db
    .prepare('INSERT INTO characters (user_id, name) VALUES (?, ?)')
    .run(req.user.id, name.trim().slice(0, 60));
