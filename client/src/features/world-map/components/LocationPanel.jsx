...
import { buildMapCreationHref } from '../../map-editor/lib/mapCreationIntent.js';
...
export default function LocationPanel({
  ...
  async function beginBoardCreation() {
    await onSave({ name, lore, kind, hidden, mapId: null });
    navigate(createBoardHref);
  }

  const createBoardHref = buildMapCreationHref({
    campaignId,
  ...
  return (
    <div className="space-y-4">
      {/* Acción principal siempre visible antes de entrar en los detalles. */}
      <button
        type="button"
        onClick={save}
        disabled={busy}
      ...
      <section className="space-y-2 border-t border-gold/15 pt-4">
        <label className={labelClass} htmlFor="loc-lore">Contenido</label>
        <textarea
          id="loc-lore"
          rows={6}
          ...
          placeholder="Texto que verá el grupo al viajar aquí (ambiente, pistas, detalles…)"
        />
      </section>

      <section className="space-y-3 border-t border-gold/15 pt-4">
        <p className={labelClass}>Visibilidad y destino</p>
        <label className="flex items-start gap-2 text-sm text-bone/80">
          <input className="mt-0.5" type="checkbox" checked={hidden} onChange={(e) => setHidden(e.target.checked)} />
          <span>
            Oculta para los jugadores
            <span className="mt-0.5 block text-[0.65rem] text-bone/40">Se revela cuando el grupo viaja aquí.</span>
          </span>
        </label>
        ...
        {isCity ? (
          ...
          <div>
            ...
            {maps.length > 0 ? (
              ...
              <div className="rounded-md border border-dashed border-gold/25 bg-gold/5 p-3 text-center">
                <p className="text-xs text-bone/60">Todavía no hay ningún tablero táctico.</p>
                <button
                  type="button"
                  disabled={busy}
                  onClick={() => beginBoardCreation().catch(() => {})}
                  className="mt-2 inline-flex rounded-sm bg-gold px-3 py-2 font-display text-sm text-night-950 hover:bg-gold/90"
                >
                  Crear «{name.trim() || 'Nuevo tablero'}» y enlazarlo →
                </button>
                <p className="mt-2 text-[0.65rem] text-bone/40">Volverás al mapa de mundo desde la cabecera del editor.</p>
              </div>
            )}
          </div>
        )}

        {location.mapId && !isCity && (
        <Link
          to={`/campanas/${campaignId}/editor?mapa=${location.mapId}&volver=mundo`}
