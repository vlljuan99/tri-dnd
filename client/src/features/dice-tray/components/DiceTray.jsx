...
import { createTumble, duracionTotal } from '../lib/tumble.js';
...
//
// Solo está montada mientras los dados vuelan: fuera de una tirada no hay
// canvas ni bucle de render.
...
// Cuánto se queda el resultado en pantalla después de parar.
const REPOSO_MS = 850;
const REPOSO_DRAMATICO_MS = 1500;
...
function Die({ spec, index, count, seed, onSettled }) {
  const groupRef = useRef(null);
  const startRef = useRef(null);
  const avisadoRef = useRef(false);
  const geometry = useMemo(() => buildDieGeometry(spec.die), [spec.die]);
  const tumble = useMemo(
    () => createTumble({ die: spec.die, value: spec.value, index, count, seed }),
    [spec.die, spec.value, index, count, seed]
  );
 ...
 */
export default function DiceTray({ roll, rollId, onSettled, autorNombre = null }) {
  const dados = useMemo(() => dadosDeTirada(roll), [roll]);
  const [oculto, setOculto] = useState(false);
  const asentadosRef = useRef(0);

  const dramatico = Boolean(roll?.crit || roll?.fumble);
  ...
  useEffect(() => {
    ...
    const total = duracionTotal(dados.length) + (dramatico ? REPOSO_DRAMATICO_MS : REPOSO_MS);
    const timer = setTimeout(() => setOculto(true), total);
    return () => clearTimeout(timer);
  }, [dados.length, dramatico, rollId]);

  if (!dados.length || oculto) return null;

  return (
    <div className="pointer-events-none fixed inset-0 z-[60] flex flex-col items-center justify-center">
      {autorNombre && (
        <p className="mb-1 font-display text-sm uppercase tracking-[0.2em] text-gold/80 drop-shadow">
          {autorNombre}
        </p>
      )}
      <div className="h-[min(60vh,26rem)] w-[min(92vw,44rem)]">
        <Canvas
          dpr={[1, 1.5]}
